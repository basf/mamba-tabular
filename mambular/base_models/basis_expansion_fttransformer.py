...
from ..arch_utils.poly_layer import ScaledPolynomialLayer, ScaledSplineLayer
from ..configs.basisfttransformer_config import DefaultBasisFTTransformerConfig
from .basemodel import BaseModel
from sklearn.preprocessing import MinMaxScaler
import numpy as np


class BasisFTTransformer(BaseModel):
    """
    ...
    **kwargs : dict
    ...
    def __init__(
        ...
        num_feature_info,
        num_classes=1,
        config: DefaultBasisFTTransformerConfig = DefaultBasisFTTransformerConfig(),
        **kwargs,
    ):
        ...
        for feature_name, input_shape in num_feature_info.items():
            if input_shape == config.d_model:
                self.num_embeddings.append(nn.Identity())
            else:
                self.num_embeddings.append(nn.Linear(1, 1))
    ...
    def forward(self, num_features, cat_features):
        ...
        if len(self.num_embeddings) > 0 and num_features:
            device = num_features[
                0
            ].device  # Assuming all num_features tensors are on the same device
            scaler = MinMaxScaler()
            num_embeddings = [
                torch.tensor(
                    scaler.fit_transform(num_features[i].cpu().numpy()) * 0.5,
                    dtype=torch.float32,
